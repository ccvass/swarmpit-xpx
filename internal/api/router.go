...
func NewRouter(staticFS fs.FS) http.Handler {
	...
	idx := strings.Replace(string(rawIndex), "</head>", `<link rel="stylesheet" href="/css/main.css?v=2.2.1"></head>`, 1)
	idx = strings.Replace(idx, "</body>", `<script src="/js/main.js?v=2.2.1"></script></body>`, 1)
	indexHTML := []byte(idx)
